<template>
  <div class="home">
    ...
    <el-dropdown @command="handleCommand" v-if="this.$store.state.user.level == 1"
                 split-button
                 type="primary"
                 size="small"
                 style="position: absolute;right: 8%;top: 10px">
      菜单列表
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item command="a" v-if="this.$store.state.user.isSuper">用户管理</el-dropdown-item>
        <el-dropdown-item command="b" v-if="this.$store.state.user.isSuper">单位管理</el-dropdown-item>
        <el-dropdown-item command="c" >统计查询</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>

    <el-button class="export-button" type="primary" size="small" @click="exportDown">导出Excel</el-button>
    <el-button class="batch-button" type="primary" size="small" @click="openDown">批量下载</el-button>
    <el-dialog
      title="删除提醒"
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div>
            <a href="#" @click="goDetail(scope.row)" style="text-decoration: none">查看记录</a>
          </div>
        </template>
      </el-table-column>
      <el-table-column
  ...
  export default {
    ...
    methods: {
      ...
      update(filters = null) {
        ...
        // console.log(filters.disclass,"disclass")
        this.disclass=sessionStorage.getItem('OrganSelect')
        request.getCaseList(filters, data => {
          let cases = []
      ...
      lookDetailClick(row) {
        this.$router.push({name: 'detail', query: {'queryId': row.id, 'disClass': row.disClass, 'birthTime': row.birthTime}})
      },
